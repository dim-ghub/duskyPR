...
main() {
	...
	# 5. Process gtk-3.0 bookmarks file
	local bookmarks_file="${HOME}/.config/gtk-3.0/bookmarks"
	if [[ -f "$bookmarks_file" ]]; then
		if grep -q '/home/[^/]+/' "$bookmarks_file" 2>/dev/null; then
			sed -i -E "s|/home/[^/]+/|/home/${USER_SED_SAFE}/|g" "$bookmarks_file"
			log_success "Updated: gtk-3.0/bookmarks"
			((++updated_count)) || true
		else
			log_skip "gtk-3.0/bookmarks"
			((++skipped_count)) || true
		fi
	fi
